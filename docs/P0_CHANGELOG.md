...
| 服务器 | mootdx 分钟/日线 | pytdx | 判定 |
|---|---|---|---|
| **117.34.114.13/.14/.15/.16/.17/.18/.20** | 40 根 / 5 根，最新日线 2026-09-10 收 5.57 | 40/5 | **全部 DATA-OK** |
| 117.34.114.27 | 40/5 | 连接失败 | 部分可用 |
| 59.36.5.11 | — | — | TCP 失败（已下线） |
| 我们清单内（115.238.56.198 / 115.238.90.165 / 123.125.108.14 / 124.71.187.122 等） | **0 / 0（空）** | — | 坏节点 |

**误判根因（我方）**：抽样偏差——上午只测了「我们清单 10 台 + 上游 a-stock-data 清单 9 台」共 19 台，**恰好全部落在坏节点集合**，于是把"清单问题"误判为"服务端全局故障"，并据此把生产降级到腾讯备胎。
真实情况：可用节点（117.34.114.* 段）从未进入任何清单；mootdx 0.11.7（底层 tdxpy）与 pytdx 1.72 双库均正常取数。

**处置（已完成）**：
1. **13 处服务器清单更新**（可复现脚本化迁移，逐文件 py_compile 校验）：yaoban-system 12 处（preflight/tick_monitor/scan_and_confirm/monitor_intraday/close_pipeline/fetch_daily_minute_rebuild/tdx_recovery_probe/collect_tick_daily/pull_intraday/fill_daily_all_0827/fill_daily_history/stress_tdx_tick + src/data/minute.py）+ Vibe-Research `mootdx_src.py`；可用段置顶、旧节点保留尾部备用；
2. **TDX 主路径恢复**：rebuild dry-run → 市场最新交易日 2026-09-10（不再降级腾讯）；
3. **preflight TDX 行情检查转 PASS**：`ok=True critical=True server=117.34.114.13:7709 bars=2 last=2026-09-10 15:00`（此前为备胎 WARN）；
4. **恢复探测转 up**：`tdx_recovery_probe` → state=up，并触发 tdx-recovered 飞书通知；
5. **TDX 主路径同事故模式修复**：rebuild 的 TDX 分支原为"覆盖写自己的 ~200 日聚合"，会截断存量更长历史（如 3.3 年/800 行）——已与降级分支统一为**合并写**；
6. 晚间补跑 TDX 主路径 rebuild，把同花顺恢复的 141 行提升到 TDX 口径 ~200 行（存量 800 行文件经合并保留不缩水）。

**教训（新增）**：
1. **样本偏差是排查的头号陷阱**：候选池子集全失败 ≠ 全局故障。判定"源挂"前必须证明**候选池本身覆盖了可用节点**（本次：先枚举全部 103 条候选并全量验活，再做结论）。
2. **服务器清单是活的**：TDX 节点会轮换/失效，需定期（建议每周）全量验活并自动更新清单，纳入盘后链或独立任务。
3. **降级与主源必须同源验证**：本次降级动作本身正确（保住了交易），但建立在错误前提上——若上午先做全量候选验活，则无需降级。
